...
const SubjectContentPage = () => {
  ...
  // Unified refetch for soft refresh and initial load
  const refetchAll = useCallback(async () => {
    setLoading(true)
    try {
      let workingStudent = student
      if (!workingStudent && studentId) {
        workingStudent = await getStudentBySlug(studentId)
        setStudent(workingStudent)
      }
      if (workingStudent?.grade_level && (subjectData || subject)) {
        const stage = curriculumService.getStageForGrade(workingStudent.grade_level)
        await curriculumService.load(stage)
        const subjectToUse = subjectData || { code: subject }
        const subjectOutcomes = await curriculumService.getOutcomes(stage, subjectToUse.code)
  ...
  }, [student, studentId, subjectData, subject, isOffline])
